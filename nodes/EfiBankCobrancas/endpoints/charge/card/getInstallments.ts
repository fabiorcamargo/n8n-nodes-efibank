...
export async function getInstallments(
  ...
  try {
    ...
    const total = context.getNodeParameter('total', index) as number;
    const brand = context.getNodeParameter('brand', index) as 'visa' | 'mastercard' | 'amex' | 'elo' | 'hipercard';
    ...
    const resposta = await efipay.getInstallments(params);
