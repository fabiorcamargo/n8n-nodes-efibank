...
export async function createOneStepBilletSubscription(
  ...
  try {
    ...
    const plan_id = context.getNodeParameter('plan_id', index) as string;

    const params = {
      id: parseInt(plan_id),
    };

    const requestBody = context.getNodeParameter('requestBodyInscricaoOneStepBoleto', index) as string;
    const body = JSON.parse(requestBody);

    const resposta = await efipay.createOneStepSubscription(params, body);
