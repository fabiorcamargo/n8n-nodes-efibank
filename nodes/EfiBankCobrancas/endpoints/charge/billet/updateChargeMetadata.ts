...
export async function updateChargeMetadata(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyUpdateMetadata', index) as string;
    ...
    const resposta = await efipay.updateChargeMetadata(params, body);
    return resposta;
  } catch (error: any) {
    if (error?.error) {
