...
export async function pixCreateRecurrence(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixCreateRecurrence', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixCreateRecurrence({}, body);
    return resposta;
  } catch (error: any) {
