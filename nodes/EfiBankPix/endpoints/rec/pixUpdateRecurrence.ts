...
export async function pixUpdateRecurrence(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixUpdateRecurrence', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixUpdateRecurrence({ idRec }, body);
    return resposta;
  } catch (error: any) {
