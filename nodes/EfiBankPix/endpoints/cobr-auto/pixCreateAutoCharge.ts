...
export async function pixCreateAutoCharge(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixCreateAutoCharge', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixCreateAutoCharge({ txid }, body);
    return resposta;
  } catch (error: any) {
