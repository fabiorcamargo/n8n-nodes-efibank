...
export async function pixUpdateAutoCharge(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixUpdateAutoCharge', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixUpdateAutoCharge({ txid }, body);
    return resposta;
  } catch (error: any) {
