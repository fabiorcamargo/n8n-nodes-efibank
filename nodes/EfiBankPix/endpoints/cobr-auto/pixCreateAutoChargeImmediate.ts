...
export async function pixCreateAutoChargeImmediate(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixCreateAutoChargeImmediate', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixCreateAutoChargeImmediate({}, body);
    return resposta;
  } catch (error: any) {
