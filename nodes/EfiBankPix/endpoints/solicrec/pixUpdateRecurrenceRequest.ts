...
export async function pixUpdateRecurrenceRequest(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixUpdateRecurrenceRequest', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixUpdateRecurrenceRequest({ idSolicRec }, body);
    return resposta;
  } catch (error: any) {
