...
export async function pixCreateRecurrenceRequest(
  ...
  try {
    ...
    const requestBody = context.getNodeParameter('requestBodyPixCreateRecurrenceRequest', index) as string;

    const body = JSON.parse(requestBody);

    const resposta = await efipay.pixCreateRecurrenceRequest({}, body);
    return resposta;
  } catch (error: any) {
